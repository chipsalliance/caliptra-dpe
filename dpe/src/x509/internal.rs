...
use super::{CertWriter, DirectoryString, KeyUsageFlags, MeasurementData, Name};
...
pub(super) const SIZE_TAG_OFFSET: usize = 3;
...
pub struct CertWriterInternal<'a> {
    pub(crate) certificate: &'a mut [u8],
    pub(crate) profile: DpeProfile,
    pub(crate) offset: usize,
    pub(crate) crit_dice: bool,
    pub(crate) csr_range: Option<(usize, usize)>,
    pub(crate) backtracks: ArrayVec<(usize, usize), MAX_BACKTRACKS>,
    pub(crate) saved_offset: Option<usize>,
}

impl CertWriterInternal<'_> {
    const BOOL_TAG: u8 = 0x1;
    const INTEGER_TAG: u8 = 0x2;
    const BIT_STRING_TAG: u8 = 0x3;
    pub(super) const OCTET_STRING_TAG: u8 = 0x4;
    const OID_TAG: u8 = 0x6;
    const UTF8_STRING_TAG: u8 = 0xC;
    const PRINTABLE_STRING_TAG: u8 = 0x13;
    #[cfg(not(feature = "disable_x509"))]
    const GENERALIZE_TIME_TAG: u8 = 0x18;
    pub(super) const SEQUENCE_TAG: u8 = 0x30;
    pub(super) const SEQUENCE_OF_TAG: u8 = 0x30;
    pub(super) const SET_OF_TAG: u8 = 0x31;

    const BOOL_SIZE: usize = 1;

    // Constants for setting tag bits
    pub(super) const CONTEXT_SPECIFIC: u8 = 0x80; // Used for Implicit/Explicit tags
    pub(super) const CONSTRUCTED: u8 = 0x20; // SET{OF} and SEQUENCE{OF} have this bit set
    ...
    pub(super) const RDN_COMMON_NAME_OID: [u8; 3] = [0x55, 0x04, 0x03];
    pub(super) const RDN_SERIALNUMBER_OID: [u8; 3] = [0x55, 0x04, 0x05];
    ...
    #[cfg(not(feature = "disable_csr"))]
    pub(super) const ID_SIGNED_DATA_OID: &'static [u8] =
        &[0x2A, 0x86, 0x48, 0x86, 0xF7, 0x0D, 0x01, 0x07, 0x02];

    // RFC 5652 1.2.840.113549.1.7.1
    #[cfg(not(feature = "disable_csr"))]
    pub(super) const ID_DATA_OID: &'static [u8] =
    ...
    ///
    /// The new offset.
    pub(super) fn push_backtrack(&mut self, skip: usize) -> Result<usize, DpeErrorCode> {
        self.backtracks
    ...
    pub(super) fn pop_backtrack(&mut self, expected_width: usize) -> Result<(), DpeErrorCode> {
    ...
    ///
    /// NOTE: `start_backtrack` MUST be called before `pop_backtrack`.
    pub(super) fn start_backtrack(&mut self) -> Result<(), DpeErrorCode> {
    ...
    pub(super) fn end_backtrack(&mut self) -> Result<(), DpeErrorCode> {
    ...
    pub(super) fn get_size_width(size: usize) -> usize {
    ...
    pub(super) fn get_structure_size(data_size: usize, tagged: bool) -> usize {
        if tagged {
    ...
    /// If `tagged`, include the tag and size fields
    pub(super) fn get_rdn_size(name: &Name, tagged: bool) -> usize {
    ...
    /// If `tagged`, include the tag and size fields
    #[cfg(not(feature = "disable_csr"))]
    pub(super) fn get_hash_alg_id_size(&self, tagged: bool) -> Result<usize, DpeErrorCode> {
    ...
    /// If `tagged`, include the tag and size fields
    #[cfg(not(feature = "disable_csr"))]
    pub(super) fn get_certification_request_info_size(
        &self,
        subject_name: &Name,
        pubkey: &PubKey,
        measurements: &MeasurementData,
    ...
    /// If `tagged`, include the tag and size fields
    #[cfg(not(feature = "disable_csr"))]
    pub(super) fn get_signer_info_size(
        &self,
        sig: &Signature,
    ...
    /// If `tagged`, include the tag and size fields
    #[cfg(not(feature = "disable_csr"))]
    pub(super) fn get_signed_data_size(
        &self,
        csr: &[u8],
        sig: &Signature,
        sid: &SignerIdentifier,
    ...
    #[cfg(not(feature = "disable_csr"))]
    pub(super) fn get_econtent_size(bytes_size: usize, tagged: bool, explicit: bool) -> usize {
        // Determine whether to include the explicit tag wrapping in the size calculation
    ...
    pub(super) fn encode_bytes(&mut self, bytes: &[u8]) -> usize {
        let size = bytes.len();
    ...
    pub(super) fn encode_byte(&mut self, byte: u8) -> usize {
    ...
    pub(super) fn encode_tag_field(&mut self, tag: u8) -> usize {
        self.encode_byte(tag)
    }

    /// DER-encodes the size field of an ASN.1 type
    pub(super) fn encode_size_field(&mut self, size: usize) -> usize {
    ...
    pub(super) fn check_not_truncated(&self) -> Result<(), DpeErrorCode> {
    ...
    pub(super) fn encode_integer_bytes(&mut self, integer: &[u8], tagged: bool) -> usize {
    ...
    pub(super) fn encode_integer(&mut self, integer: u64, tagged: bool) -> usize {
    ...
    pub(super) fn encode_oid(&mut self, oid: &[u8]) -> usize {
    ...
    pub(super) fn encode_rdn_string(&mut self, s: &DirectoryString) -> usize {
        let (val, tag) = match s {
    ...
    ///     parameters  ECParameters
    ///     }
    pub(super) fn encode_ecdsa_sig_alg_id(&mut self) -> Result<usize, DpeErrorCode> {
    ...
    ///     }
    #[cfg(not(feature = "disable_csr"))]
    pub(super) fn encode_hash_alg_id(&mut self) -> Result<usize, DpeErrorCode> {
    ...
    #[cfg(not(feature = "disable_x509"))]
    pub(super) fn encode_validity(&mut self, validity: &CertValidity) -> usize {
    ...
    pub(super) fn encode_ecdsa_subject_pubkey_info(
        &mut self,
        pub_key: &EcdsaPubKey,
    ) -> Result<usize, DpeErrorCode> {
    ...
    #[cfg(not(feature = "disable_csr"))]
    pub(super) fn encode_signature_bit_string(
        &mut self,
        sig: &Signature,
    ) -> Result<usize, DpeErrorCode> {
    ...
    #[cfg(not(feature = "disable_csr"))]
    pub(super) fn encode_signature_octet_string(
        &mut self,
        sig: &Signature,
    ) -> Result<usize, DpeErrorCode> {
        let bytes_written = match sig {
    ...
    #[cfg(feature = "ml-dsa")]
    pub(super) fn encode_mldsa_sig_alg_id(&mut self) -> Result<usize, DpeErrorCode> {
    ...
    #[cfg(feature = "ml-dsa")]
    pub(super) fn encode_mldsa_subject_pubkey_info(
        &mut self,
    ...
    pub(super) fn encode_version(&mut self) -> usize {
    ...
    #[cfg(not(feature = "disable_csr"))]
    pub(super) fn encode_cms_version(&mut self, sid: &SignerIdentifier) -> usize {
        match sid {
            SignerIdentifier::IssuerAndSerialNumber {
                issuer_name: _,
                serial_number: _,
    ...
    #[allow(clippy::identity_op)]
    #[cfg(not(feature = "disable_csr"))]
    pub(super) fn encode_attributes(
        &mut self,
        measurements: &MeasurementData,
    ) -> Result<usize, DpeErrorCode> {
    ...
    /// }
    #[cfg(not(feature = "disable_csr"))]
    pub(super) fn encode_signer_identifier(&mut self, sid: &SignerIdentifier) -> usize {
        match sid {
            SignerIdentifier::IssuerAndSerialNumber {
                issuer_name,
                serial_number,
...
pub(crate) mod tests {
    ...
    fn test_integers() {
        ...
        for c in buffer_cases {
            ...
            let byte_count = w.internal.encode_integer_bytes(&c, true);
            let n = asn1::parse_single::<u64>(&cert[..byte_count]).unwrap();
            assert_eq!(n, u64::from_be_bytes(c));
            assert_eq!(
                CertWriterInternal::get_integer_bytes_size(&c, true),
                byte_count
            );
        }
        ...
        for c in integer_cases {
            let mut cert = [0; 128];
            let mut w = CertWriter::new(&mut cert, DPE_PROFILE, true);
            let byte_count = w.internal.encode_integer(c, true);
            let n = asn1::parse_single::<u64>(&cert[..byte_count]).unwrap();
            assert_eq!(n, c);
            assert_eq!(CertWriterInternal::get_integer_size(c, true), byte_count);
        }
    }
    ...
    fn test_subject_pubkey() {
        ...
        let bytes_written = w
            .internal
            .encode_ecdsa_subject_pubkey_info(&test_key)
            .unwrap();
    ...
    fn test_subject_pubkey() {
        ...
        let bytes_written = w
            .internal
            .encode_mldsa_subject_pubkey_info(&test_key)
            .unwrap();
    ...
    fn test_tcb_info() {
        ...
        let mut bytes_written = w
            .internal
            .encode_tcb_info(&node, supports_recursive)
            .unwrap();
        ...
        assert_eq!(
            ir.register_num.unwrap(),
            CertWriterInternal::INTEGRITY_REGISTER_NUM
        );
    ...
    fn get_key_usage(is_ca: bool) -> KeyUsage {
        let mut cert = [0u8; 32];
        let mut w = CertWriter::new(&mut cert, DPE_PROFILE, true);
        let bytes_written = w.internal.encode_key_usage(is_ca);
        assert_eq!(
            bytes_written,
            CertWriterInternal::get_key_usage_size(/*tagged=*/ true)
        );
    ...
    pub(crate) const TEST_SERIAL: &[u8] = &[0x1F; 20];
    pub(crate) const TEST_ISSUER_NAME: Name = Name {
        cn: DirectoryString::PrintableString(b"Caliptra Alias"),
        serial: DirectoryString::PrintableString(&[0x00; DPE_PROFILE.hash_size() * 2]),
    };
    pub(crate) const TEST_SUBJECT_NAME: Name = Name {
        cn: DirectoryString::PrintableString(b"DPE Leaf"),
        serial: DirectoryString::PrintableString(&[0x00; DPE_PROFILE.hash_size() * 2]),
    };

    pub(crate) const ECC_INT_SIZE: usize = DPE_PROFILE.ecc_int_size();

    pub(crate) const DEFAULT_OTHER_NAME_OID: &[u8] = &[0, 0, 0];
    pub(crate) const DEFAULT_OTHER_NAME_VALUE: &str = "default-other-name";
    ...
    fn test_encode_multi_tcb_info_bytes() {
        ...
        let n = w.internal.encode_multi_tcb_info(&measurements).unwrap();
    ...
    fn test_encode_ueid_bytes() {
        ...
        let mut buf = [0u8; 1024];
        let mut w = CertWriter::new(&mut buf, DPE_PROFILE, true);
        let n = w.internal.encode_ueid(&measurements);
    ...
    fn test_encode_basic_constraints_bytes() {
        ...
        let mut buf = [0u8; 1024];
        let mut w = CertWriter::new(&mut buf, DPE_PROFILE, true);
        let n = w.internal.encode_basic_constraints(&measurements);
    ...
    fn test_encode_key_usage_bytes() {
        let mut buf = [0u8; 1024];
        let mut w = CertWriter::new(&mut buf, DPE_PROFILE, true);
        let n = w.internal.encode_key_usage(true);
    ...
    fn test_encode_extended_key_usage_bytes() {
        ...
        let mut buf = [0u8; 1024];
        let mut w = CertWriter::new(&mut buf, DPE_PROFILE, true);
        let n = w.internal.encode_extended_key_usage(&measurements);
    ...
    fn test_encode_subject_alt_name_extension_bytes() {
        ...
        let mut buf = [0u8; 1024];
        let mut w = CertWriter::new(&mut buf, DPE_PROFILE, true);
        let n = w.internal.encode_subject_alt_name_extension(&measurements);
    ...
    fn test_encode_cms_rejects_unresolved_backtrack() {
        ...
        w.internal.push_backtrack(super::SIZE_TAG_OFFSET).unwrap();
