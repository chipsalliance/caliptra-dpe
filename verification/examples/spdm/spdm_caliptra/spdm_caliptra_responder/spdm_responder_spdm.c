...
libspdm_return_t spdm_responder_device_receive_message(void *spdm_context,
                                                       size_t *request_size,
                                                       void **request,
                                                       uint64_t timeout)
{
...
void *spdm_server_init(void)
{
    ...
    libspdm_register_device_io_func(spdm_context, spdm_device_send_message,
                                    spdm_responder_device_receive_message);
    ...
    void *spdm_context, libspdm_connection_state_t connection_state)
{
    ...
    case LIBSPDM_CONNECTION_STATE_NEGOTIATED:
        ...
        res = caliptra_read_responder_public_certificate_chain(m_use_hash_algo,
                                                              m_use_asym_algo,
                                                              &data, &data_size,
                                                              NULL, NULL);
        res = caliptra_read_responder_public_certificate_chain_per_slot(1,
                                                                       m_use_hash_algo,
                                                                       m_use_asym_algo,
                                                                       &data1, &data1_size,
                                                                       NULL, NULL);
        ...
        if (m_use_req_asym_algo != 0)
        {
            ...
            {
                res = caliptra_read_requester_root_public_certificate(
