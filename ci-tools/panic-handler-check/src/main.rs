// Licensed under the Apache-2.0 license

#[cfg(not(any(feature = "p256", feature = "p384", feature = "ml-dsa")))]
compile_error!("select one of the features p256, p384, ml-dsa");

mod symbol_checker;

use std::hint::black_box;
use std::process::{Command, Stdio};

use crypto::dummy::DummyCrypto;
use dpe::dpe_instance::{DpeEnv, DpeTypes};
use dpe::support::Support;
use dpe::DpeFlags;
use dpe::DpeInstance;
use dpe::DpeProfile;
use platform::dummy::{DefaultPlatform, DefaultPlatformProfile};

// Build all test binaires
/*
fn get_test_binaries() -> Result<Vec<String>, Box<dyn std::error::Error>> {
    let output = Command::new("cargo")
        .args([
            "test",
            "--release",
            "--workspace",
            "--no-run",
            "--message-format=json",
        ])
        .stdout(Stdio::piped())
        .stderr(Stdio::null())
        .output()?;

    if !output.status.success() {
        return Err(format!("cargo test --no-run failed with status: {}", output.status).into());
    }

    let stdout = String::from_utf8(output.stdout)?;

    let binaries: Vec<String> = stdout
        .lines()
        .filter_map(|line| {
            let msg: serde_json::Value = serde_json::from_str(line).ok()?;
            if msg.get("reason")?.as_str()? != "compiler-artifact" {
                return None;
            }
            if !(msg.get("profile")?.get("test")?.as_bool()?) {
                return None;
            }
            msg.get("executable")?.as_str().map(String::from)
        })
        .collect();

    Ok(binaries)
}

pub(crate) fn check_symbols() {
    let test_bianries = get_test_binaries().unwrap();
    for b in test_bianries {
        if symbol_checker::elf_sym_contain_one_of(
            path::PathBuf::from(&b),
            vec!["panic_is_possible"],
        )
        .unwrap()
        {
            eprintln!("binary {b} contains panic handler!");
        } else {
            println!("binary {b} does not contain panick symbol");
        }
    }
}
*/

struct SimTypes {}

impl DpeTypes for SimTypes {
    type Crypto<'a> = DummyCrypto;

    type Platform<'a> = DefaultPlatform;
}

fn main() {
    let mut flags = DpeFlags::empty();
    flags.set(DpeFlags::MARK_DICE_EXTENSIONS_CRITICAL, true);

    // check_symbols();

    #[cfg(feature = "p256")]
    let platform_profile = DefaultPlatformProfile::P256;
    #[cfg(feature = "p256")]
    let dpe_profile = DpeProfile::P256Sha256;
    #[cfg(feature = "p384")]
    let platform_profile = DefaultPlatformProfile::P384;
    #[cfg(feature = "p384")]
    let dpe_profile = DpeProfile::P384Sha384;
    #[cfg(feature = "ml-dsa")]
    let platform_profile = DefaultPlatformProfile::Mldsa87;
    #[cfg(feature = "ml-dsa")]
    let dpe_profile = DpeProfile::Mldsa87;

    let mut env = DpeEnv::<SimTypes> {
        crypto: <SimTypes as DpeTypes>::Crypto::new(),
        platform: DefaultPlatform(platform_profile),
        state: &mut dpe::State::new(Support::default(), flags),
    };
    let mut dpe = DpeInstance::new(&mut env, dpe_profile).unwrap();
    test_execute_serialized_command(&mut dpe, &mut env);
    test_roll_onetime_use_handle(&mut dpe, &mut env);
    test_get_profile(&mut dpe, &mut env);
    test_deserialize_command(&mut dpe);

    let _ = black_box(rerun_with_release_profile(8));
}

#[no_panic::no_panic]
fn test_execute_serialized_command(dpe: &mut DpeInstance, env: &mut DpeEnv<SimTypes>) {
    let _ = dpe.execute_serialized_command(env, 1, &[0]);
}

#[no_panic::no_panic]
fn test_roll_onetime_use_handle(dpe: &mut DpeInstance, env: &mut DpeEnv<SimTypes>) {
    let _ = dpe.roll_onetime_use_handle(env, 1);
}

#[no_panic::no_panic]
fn test_get_profile(dpe: &mut DpeInstance, env: &mut DpeEnv<SimTypes>) {
    let _ = dpe.get_profile(&mut env.platform, env.state.support);
}

#[no_panic::no_panic]
fn test_deserialize_command(dpe: &mut DpeInstance) {
    let _ = dpe.deserialize_command(&[0]);
}

/// Sanitiy check to verify that `no_panic` correctly detects the absence of panics in this function
///
/// If this fails you might have to compile with more optimizations.
/// (Build with `--release` profile.)
#[no_panic::no_panic]
fn rerun_with_release_profile(a: usize) -> Result<u8, ()> {
    let buf = [0; 5];
    if a >= buf.len() {
        return Err(());
    } else {
        return Ok(buf[a]);
    }
}
